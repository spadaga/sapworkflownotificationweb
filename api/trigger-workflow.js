export default async function handler(req, res) {
       ...
       console.log("📥 [TRIGGER] Request headers:", JSON.stringify(req.headers, null, 2));
       ...
       try {
         ...
         const workflowData = {
           ...
           CREATED_ON: req.body?.createdOn || "2025-06-10T11:05:47.000",
     ...
     function createTeamsInlineCard(workflow, isLiveData = true) {
       ...
       try {
               ...
               items: [
                 {
                   ...
                   columns: [
                     ...
                     { type: "Column", width: "stretch", items: [{ type: "TextBlock", text: safeWorkflow.Status, size: "small", color: "good" }] },
                   ],
                 },
                 {
                   type: "ColumnSet",
                   columns: [
                     { type: "Column", width: "auto", items: [{ type: "TextBlock", text: "**Instance ID:**", size: "small" }] },
                     { type: "Column", width: "stretch", items: [{ type: "TextBlock", text: safeWorkflow.InstanceID, size: "small" }] },
                   ],
                 },
                 {
                   type: "ColumnSet",
                   columns: [
                     { type: "Column", width: "auto", items: [{ type: "TextBlock", text: "**Created By:**", size: "small" }] },
                     { type: "Column", width: "stretch", items: [{ type: "TextBlock", text: safeWorkflow.CreatedByName, size: "small" }] },
                   ],
                 },
                 {
                   type: "ColumnSet",
                   columns: [
                     { type: "Column", width: "auto", items: [{ type: "TextBlock", text: "**Created On:**", size: "small" }] },
                     { type: "Column", width: "stretch", items: [{ type: "TextBlock", text: formattedDate, size: "small" }] },
                   ],
                 },
               ],
               style: "emphasis",
           ...
           actions: [
             {
               type: "Action.Submit",
               title: "✅ Approve",
               data: { action: "approve", instanceId: safeWorkflow.InstanceID, taskTitle: safeWorkflow.TaskTitle, timestamp: new Date().toISOString() },
               style: "positive",
             },
             {
               type: "Action.Submit",
               title: "❌ Reject",
               data: { action: "reject", instanceId: safeWorkflow.InstanceID, taskTitle: safeWorkflow.TaskTitle, timestamp: new Date().toISOString() },
               style: "destructive",
             },
         ...
         return { type: "message", text: `❌ Error creating workflow notification: ${error.message}` };
       }
     }